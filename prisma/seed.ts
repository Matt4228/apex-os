...
import { addDays, subDays, subMonths, format, eachDayOfInterval, startOfWeek, endOfWeek } from "date-fns"
...
async function main() {
  ...
  const goals = await Promise.all([
    prisma.goal.create({
      data: {
        userId: user.id,
        title: "Complete 200 LeetCode problems",
        description: "Focus on medium difficulty — arrays, trees, dynamic programming",
        category: "Career",
        status: "active",
        targetValue: 200,
        unit: "problems",
    ...
    prisma.goal.create({
      data: {
        ...
        description: "Training for the NYC Half in March",
        category: "Fitness",
        status: "active",
        targetValue: 21.1,
        unit: "km longest run",
    ...
    prisma.goal.create({
      data: {
        userId: user.id,
        title: "Ship open source contribution",
        description: "Meaningful PR merged to a project with 1k+ stars",
        category: "Career",
        status: "active",
        targetValue: 3,
        unit: "PRs merged",
    ...
    prisma.goal.create({
      data: {
        userId: user.id,
        title: "Read 12 technical books this year",
        description: "Mix of system design, architecture, and engineering culture",
        category: "Learning",
        status: "active",
        targetValue: 12,
        unit: "books",
    ...
    prisma.goal.create({
      data: {
        ...
        description: "Associate level — study 1hr/day for 3 months",
        category: "Career",
        status: "complete",
        targetValue: 1,
        unit: "cert",
  ...
  // ── GOAL ENTRIES ───────────────────────────────────────────────────────────
  // LeetCode — steady progress with some weeks better than others
  const leetcodeEntries = Array.from({ length: 16 }, (_, i) => ({
    goalId: goals[0].id,
    value: (i + 1) * 8 + Math.floor(Math.random() * 6) - 2,
    note: i % 4 === 0 ? "Focused on dynamic programming this week" :
          i % 4 === 1 ? "Tree traversal practice" :
          i % 4 === 2 ? "Graph problems — harder than expected" : null,
    loggedOn: subDays(now, (15 - i) * 7),
  }))

  // Running — progressive overload with a dip around week 8
  const runEntries = Array.from({ length: 16 }, (_, i) => ({
    goalId: goals[1].id,
    value: parseFloat((8 + i * 0.8 + (i === 7 ? -2 : 0) + (Math.random() * 0.5)).toFixed(1)),
    note: i === 7 ? "Pulled back — felt some knee soreness" :
          i === 8 ? "Back on track, felt great" :
          i === 15 ? "Longest run yet, feeling ready" : null,
    loggedOn: subDays(now, (15 - i) * 7),
  }))

  // Open source PRs
  const osEntries = [
    { goalId: goals[2].id, value: 1, note: "Fixed a docs typo and added missing test coverage — small but merged!", loggedOn: subMonths(now, 3) },
  ...
  const bookEntries = Array.from({ length: 7 }, (_, i) => ({
    ...
    value: i + 1,
    note: [
      "Finished Designing Data-Intensive Applications — best technical book I've read",
      "Finished The Pragmatic Programmer",
      "Finished Clean Architecture",
      "Finished A Philosophy of Software Design",
      "Finished Staff Engineer by Will Larson",
      "Finished System Design Interview vol 1",
      "Finished The Manager's Path — useful even as an IC",
    ][i],
  ...
  console.log("Created goal entries")
  ...
  for (const day of allDays) {
    ...
    // Run/workout — 5x/week, skip random rest days
    ...
    // LeetCode — 6x/week, very consistent
    if (Math.random() > (isEarlyMonth ? 0.25 : 0.12)) {
      habitLogs.push({ habitId: habits[1].id, completedOn: day })
    }

    // Read technical content — daily, very consistent
    if (Math.random() > 0.1) habitLogs.push({ habitId: habits[2].id, completedOn: day })

    // Side project — 3x/week, mostly weekends + one weekday
    ...
    // No phone first hour — gets better over time
    const phoneSuccessRate = isEarlyMonth ? 0.45 : 0.2
    if (Math.random() > phoneSuccessRate) habitLogs.push({ habitId: habits[4].id, completedOn: day })

    // System design — 2x/week, weekends
  ...
  const competition = await prisma.competition.create({
    data: {
      ...
      rulesJson: JSON.stringify({
        criteria: [
          "Complete 6+ LeetCode problems",
          "Log 1+ system design session",
          "Work on side project 2+ hours",
          "Read technical content 5+ days"
        ],
  ...
  // 16 weeks of competition entries with realistic scoring
  const competitionEntries = Array.from({ length: 16 }, (_, i) => {
    const weekNumber = i + 1
    const isEarlyWeek = weekNumber <= 3
    const score = isEarlyWeek
      ? 50 + Math.floor(Math.random() * 50)
      : 75 + Math.floor(Math.random() * 26)
    const weekComplete = score >= 100
  ...
  console.log("Created competition with entries")
